...
import { getSubjectById, getSubtopicById, getQuestionsForSubtopic } from "@/data/examData";
import QuizQuestion from "@/components/QuizQuestion";
import { Button } from "@/components/ui/button";
import ThemeToggle from "@/components/ThemeToggle";

const SubtopicQuiz = () => {
  const { subjectId, subtopicId } = useParams<{ subjectId: string; subtopicId: string }>();
  const subject = subjectId ? getSubjectById(subjectId) : null;
  const subtopic = subjectId && subtopicId ? getSubtopicById(subjectId, subtopicId) : null;
  const questions = subtopicId ? getQuestionsForSubtopic(subtopicId) : [];
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center pb-20">
        <div className="text-center">
          <h1 className="text-xl font-semibold text-foreground mb-2">Subtopic not found</h1>
      ...
      <div className="min-h-screen bg-background pb-20">
        <header className="sticky top-0 z-40 bg-card border-b border-border px-4 py-3">
          <div className="flex items-center gap-3 max-w-lg mx-auto">
            <Link to={`/subjects/${subjectId}`} className="p-2 -ml-2 hover:bg-muted rounded-lg transition-colors">
              <ArrowLeft className="w-5 h-5 text-foreground" />
            </Link>
            <div className="flex-1">
              <h1 className="text-lg font-semibold text-foreground">{subtopic.name}</h1>
        ...
        <div className="px-4 py-8 max-w-lg mx-auto text-center">
          <p className="text-muted-foreground mb-4">Questions for this subtopic will be added soon!</p>
          <Link to={`/subjects/${subjectId}`}>
            <Button>Back to {subject.name}</Button>
          </Link>
        </div>
      </div>
    );
